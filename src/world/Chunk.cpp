		...
		for (size_t y = 0; y < CHUNK_HEIGHT; ++y)
		{
			for (size_t x = 0; x < CHUNK_WIDTH; ++x)
			{
				...
				for (int face = 0; face < 6; ++face)
				{
					...
					switch (face) {
						case 0: brightness = 0.8f; break; // Front face
						case 1: brightness = 0.8f; break; // Back face
						case 2: brightness = 0.6f; break; // Right face
						case 3: brightness = 0.6f; break; // Left face
						case 4: brightness = 1.0f; break; // Top face
						case 5: brightness = 0.4f; break; // Bottom face
					}
					...
					for (int v = 0; v < 4; ++v)
					{
						...
						float r = faceData[face][baseIndex + 3] * brightness;
						float g = faceData[face][baseIndex + 4] * brightness;
						float b = faceData[face][baseIndex + 5] * brightness;
